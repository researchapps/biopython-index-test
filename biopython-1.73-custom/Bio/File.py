...
def _open_for_random_access(filename):
    ...
    """
    #### DEBUGGING #############
    print("START to Bio.File._open_for_random_access")
    ...
    print("seeked to start of file")
    #### DEBUGGING #############

    if magic == b"\x1f\x8b":

        #### DEBUGGING #############
        print("found potential gzipped file")
        #### DEBUGGING #############
    ...
    #### DEBUGGING #############
    print("END to Bio.File._open_for_random_access, returning handle %s" % handle)
    #### DEBUGGING #############
...
class _SQLiteManySeqFilesDict(_IndexedSeqFileDict):
    ...
    def __init__(self, index_filename, filenames,
        ...
        #### DEBUGGING #############
        print("START of call to Bio.File._SQLiteManySeqFilesDict")
        print("index_filename is %s" % index_filename)
        print("proxy_factory is %s" % proxy_factory)
        print("filenames are %s" % filenames)
        print("key_function is %s" % key_function)
        print("format is %s" % format)
        print("repr is %s" % repr)
        print("max_open is %s (could this be an issue?)" % max_open)
        #### DEBUGGING #############
        ...
        if filenames is not None:
            #### DEBUGGING #############
            print("filenames was likely generator, turning to list")
            #### DEBUGGING #############
        ...
        #### DEBUGGING #############
        print("Relative path of index_filename is %s" % self._relative_path)
        #### DEBUGGING #############

        if os.path.isfile(index_filename):
            #### DEBUGGING #############
            print("index_filename is a file")
            #### DEBUGGING #############
            self._load_index()
        else:
            #### DEBUGGING #############
            print("index_filename needs to be built (no file)")
            #### DEBUGGING #############
            self._build_index()

        #### DEBUGGING #############
        print("END of call to Bio.File._SQLiteManySeqFilesDict")
        #### DEBUGGING #############
    ...
    def _load_index(self):
        ...
        #### DEBUGGING #############
        print("START of call to Bio.File._load_index")
        #### DEBUGGING #############
        ...
        #### DEBUGGING #############
        print("END of call to Bio.File._load_index")
        #### DEBUGGING #############

    def _build_index(self):
        """Call from __init__ to create a new index (PRIVATE)."""
        #### DEBUGGING #############
        print("START of call to Bio.File._build_index")
        #### DEBUGGING #############
        ...
        #### DEBUGGING #############
        print("connecting to sqlite database %s" % index_filename)
        #### DEBUGGING #############
        ...
        print("CREATE TABLE meta_data (key TEXT, value TEXT);")
        print("INSERT INTO meta_data (key, value) VALUES (?,?); (\"count\", -1)")
        print("INSERT INTO meta_data (key, value) VALUES (?,?); (\"format\", %s)" % format)
        print("INSERT INTO meta_data (key, value) VALUES (?,?); (\"filenames_relative_to_index\", \"True\")")
        print("CREATE TABLE file_data (file_number INTEGER, name TEXT);")
        print("CREATE TABLE offset_data (key TEXT, file_number INTEGER, offset INTEGER, length INTEGER);")
        #### DEBUGGING #############
        ...
        #### DEBUGGING #############
        print("Starting enumeration through filenames %s" % filenames)
        #### DEBUGGING #############
        
        for i, filename in enumerate(filenames):

            #### DEBUGGING #############
            #print("count %s, filename %s" % (i, filename))
            #### DEBUGGING #############
            ...
            #### DEBUGGING #############
            print("storing %r as [%r] %r" % (filename, relative_path, f))
            print("INSERT INTO file_data (file_number, name) VALUES (?,?); (%s, xxx)" % i)
            #### DEBUGGING #############
            ...
            #### DEBUGGING #############
            print("random_access_proxy is %s" % random_access_proxy)
            #### DEBUGGING #############
 
            if key_function:

                #### DEBUGGING #############
                print("key_function is not None")
                #### DEBUGGING #############
            ...
            #### DEBUGGING #############
            print("IMPORTANT: We are about to iterate through offsets")
            #### DEBUGGING #############
            ...
            while True:
                ...
                #### DEBUGGING #############
                print("Inserting batch of %i offsets, %s ... %s" % (len(batch), batch[0][0], batch[-1][0]))
                #### DEBUGGING #############
            ...
            if len(random_access_proxies) < max_open:
                ...
                #### DEBUGGING #############
                print("length of random_access_proxies is less than max_open %s" % max_open)
                #### DEBUGGING #############

            else:
                #### DEBUGGING #############
                print("length of random_access_proxies is >= max_open %s, closing handle" % max_open)
                #### DEBUGGING #############
        ...
        try:
            ...
            #### DEBUGGING #############
            print("We triggered the error, closing connection and proxies.")
            #### DEBUGGING #############
        ...
        # print("Index created")
        #### DEBUGGING #############
        print("END of call to Bio.File._build_index")
        #### DEBUGGING #############
