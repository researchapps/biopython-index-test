...
class SeqFileRandomAccess(_IndexedSeqFileProxy):
    def __init__(self, filename, format, alphabet):
        ...
        #### DEBUGGING #############
        print("INIT of base class SeqFileRandomAccess")
        ...
        #### DEBUGGING #############
        print("Opening %s for random access" % filename)
        #### DEBUGGING #############
        ...
        #### DEBUGGING #############
        print("Loading parsing class/function once to avoid dict lookup in each __getitem__ call")
        #### DEBUGGING #############
        ...
        if alphabet is None:
            ...
            #### DEBUGGING #############
            print("Alphabet is None, note there is note here that code is 'nasty'")
            #### DEBUGGING #############

            def _parse(handle):
                #### DEBUGGING #############
                print("START of _parse for handle %s" % handle)
                #### DEBUGGING #############
                next_handle = next(i(handle))
                """Dynamically generated parser function (PRIVATE)."""
                #### DEBUGGING #############
                print("END of _parse, returning next handle %s" % next_handle)
                #### DEBUGGING #############
                return next_handle
        else:

            #### DEBUGGING #############
            print("Alphabet is defined as %s" % alphabet)
            #### DEBUGGING #############
            ...
            def _parse(handle):
                ...
                #### DEBUGGING #############
                try:
                    next_handle = next(i(handle, alphabet=alphabet))
                    #### DEBUGGING #############
                    print("next_handle is %s" % next_handle)
                    #### DEBUGGING #############
                    return next_handle
                except TypeError:
                    next_handle = next(SeqIO._force_alphabet(i(handle), alphabet))
                    #### DEBUGGING #############
                    print("catch of type error, next_handle is %s" % next_handle)
                    #### DEBUGGING #############
                    return next_handle
        self._parse = _parse
    ...
    def get(self, offset):
        ...
        #### DEBUGGING #############
        print("START of of base class SeqFileRandomAccess.get")
        #### DEBUGGING #############
        result = self._parse(StringIO(_bytes_to_string(self.get_raw(offset))))
        print("result is %s" % result)
        #### DEBUGGING #############
        print("END of of base class SeqFileRandomAccess.get")
        #### DEBUGGING ############# 
        return result
...
class FastqRandomAccess(SeqFileRandomAccess):
    ...
    def __iter__(self):
        ...
        #### DEBUGGING #############
        print("START of of FastqRandomAccess __iter__")
        #### DEBUGGING #############
        ...
        #### DEBUGGING #############
        print("Seeked to start of handle %s" % handle)
        #### DEBUGGING #############
        ...
        #### DEBUGGING #############
        #print("Start offset is %s" % start_offset)
        ...
        #### DEBUGGING #############
        print("EOF")
        print("END of of FastqRandomAccess __iter__")
        #### DEBUGGING #############
